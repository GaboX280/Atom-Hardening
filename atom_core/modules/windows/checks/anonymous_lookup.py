...
def audit_anonymous_lookup(auditor: BaseAuditor) -> None: # [TYPING ADDED]
    ...
    auditor.log("Evaluando Null Sessions...")
    ...
    if "RESTRICTNULLSESSACCESS" in resultado:
        if ": 1" in resultado or ":1" in resultado:
            auditor.add_finding(
                ...
                compliance=["CIS Controls v8 - Control 4", "NIST SP 800-53 AC-3"],
            )
        ...
        auditor.add_finding(
            ...
            recommendation=(
                "Configurar RestrictNullSessAccess para bloquear acceso anónimo."
            ),
            reference=("Microsoft Security Baseline / CIS Windows Benchmark"),
            impact=(
